...
from typing import List, Optional                              #to use List type in response model
...
router = APIRouter(
    prefix="/posts",    #this simplifies the routes from "/posts" to "/", "/posts/{id}" to "/{id}", etc 
    tags=['Posts']      #this makes the swagger documentation better by for forming groups based on tags
)

#SECURED, UPDATED GET METHOD TO HANDLE VOTES FROM VOTES TABLE
...
def get_latest_post(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    post = db.query(models.Post, func.count(models.Vote.post_id).label("votes")).join(models.Vote, models.Vote.post_id == models.Post.id, isouter=True).order_by(models.Post.created_At.desc()).group_by(models.Post.id).first()
    return post
